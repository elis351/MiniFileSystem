...
int fs_mount(mfs_fs_t * fs, char * fs_file) {
...
int fs_init_new_inode(mfs_fs_t * fs, inode_t * new_inode) {
...
int fs_lock_inode(mfs_fs_t * fs, int32_t inode_id) {
...
void fs_free_inode(mfs_fs_t * fs, int32_t inode_id) {
  bitmap_set_free(fs->inode_map.bitmap, inode_id);
  fseek(fs->fs_file, sizeof(superblock_t), SEEK_SET);
  fwrite(fs->inode_map.bitmap, sizeof(char), INODE_COUNT / 8, fs->fs_file);
}

int fs_lock_block(mfs_fs_t * fs, int32_t block_id) {
...
void fs_free_block(mfs_fs_t * fs, int32_t block_id) {
...
int fs_load_inode_by_path(mfs_fs_t * fs, const char * path, inode_t * inode) {
...
int fs_load_inode(mfs_fs_t * fs, int32_t inode_id, inode_t * inode) {
...
int fs_dump_inode(mfs_fs_t * fs, int32_t inode_id, inode_t * inode) {
  int32_t load_from = inode_id * INODE_SIZE + INODE_SECTION;
...
int32_t fs_find_inode_by_path(mfs_fs_t * fs, const char * path) {
...
int32_t fs_find_recursive(mfs_fs_t * fs, inode_t * inode, path_t * path, int path_part) {
...
int32_t fs_find_inode_in_dir(mfs_fs_t * fs, inode_t * dir, const char * name) {
  if (!dir->is_dir) {
    return ENOTADIR;
  }
...
int fs_delete_direntry_by_name(mfs_fs_t * fs, inode_t * dir, const char * name) {
  direntry_t dummy;
...
int fs_delete_file_inode(mfs_fs_t * fs, inode_t * deleted) {
...
int fs_delete_from_dir_by_name(mfs_fs_t * fs, inode_t * dir, const char * name, bool recursive) {
...
//upd дальше страшно
int fs_write(mfs_fs_t * fs, inode_t * inode, void * buf, uint32_t buf_len, uint32_t offset) {
  int block_id;
  int block_offset;
  int written_bytes = 0;
...
int fs_write_to_dir(mfs_fs_t * fs, inode_t * dir, direntry_t direntry) {
  uint32_t offset = 0;
...
int fs_read(mfs_fs_t * fs, inode_t * inode, void * dest, uint32_t dest_len, uint32_t offset) {
  int block_id;
  int block_offset;
...
int fs_read_from_dir(mfs_fs_t * fs, inode_t * dir, uint32_t offset, direntry_t * direntry) {
...
int fs_reload_current_dir(mfs_fs_t * fs) {
  fs_load_inode(fs, fs->current_directory.id, &fs->current_directory);
}

int fs_lookup_directory(mfs_fs_t * fs, const char * directory, FILE * f) {
  inode_t dir;
...
int fs_set_current_directory(mfs_fs_t * fs, const char * directory) {
...
char * fs_pwd(mfs_fs_t * fs) {
  return fs->current_directory_name;
}

int fs_create_directory(mfs_fs_t * fs, const char * directory) {
...
int fs_create_file(mfs_fs_t * fs, const char * file) {
...
int fs_delete_directory(mfs_fs_t * fs, const char * directory) {
  inode_t parent;
  char * parent_path = path_get_parent(directory);
  if (ENOTFOUND == fs_load_inode_by_path(fs, parent_path, &parent)) {
    free(parent_path);
...
int fs_delete_file(mfs_fs_t * fs, const char * file) {
  inode_t parent;
  char * parent_path = path_get_parent(file);
  if (ENOTFOUND == fs_load_inode_by_path(fs, parent_path, &parent)) {
    free(parent_path);
